...
# Test script: Player 2 — interactive battle
...
# Player 2's PC
PC='[
  {"name":"blastoise","types":["water"],"power_rank":68,"shiny":false},
  {"name":"alakazam","types":["psychic"],"power_rank":55,"shiny":false},
  {"name":"machamp","types":["fighting"],"power_rank":52,"shiny":false},
  {"name":"arcanine","types":["fire"],"power_rank":50,"shiny":false},
  {"name":"snorlax","types":["normal"],"power_rank":48,"shiny":false},
  {"name":"tyranitar","types":["rock","dark"],"power_rank":82,"shiny":false},
  {"name":"salamence","types":["dragon","flying"],"power_rank":80,"shiny":false}
]'
...
print('  Your team:     ', ', '.join(p['name'] for p in r['p2_team']))
print('  Opponent team: ', ', '.join(p['name'] for p in r['p1_team']))
print()
...
your_odds = (p2_det + 0.1) / (p1_det + p2_det + 0.2) * 100
...
print(f'  Your Power: {res[\"p2_power_total\"]}  |  Opponent Power: {res[\"p1_power_total\"]}')
print(f'  Your Type Adv: {res[\"p2_type_bonus\"]:.3f}  |  Opponent Type Adv: {res[\"p1_type_bonus\"]:.3f}')
print(f'  {odds_color}\033[1mYour odds: {your_odds:.0f}%\033[0m  |  RNG Roll: {res[\"rng_roll\"]:.3f}')
print()

winner = res['winner']
if winner == 'player-2-test':
    print(f'  \033[32m\033[1mYou won this round!\033[0m')
else:
    print(f'  \033[31m\033[1mYou lost this round.\033[0m')
print(f'  Series: {data[\"p2_wins\"]}-{data[\"p1_wins\"]}')
"
      break
    fi
  ...
  if [ "$BSTATUS" = "complete" ]; then
    ...
    if [ "$WINNER" = "$TOKEN" ]; then
      echo -e "${GREEN}${BOLD}You won the battle! ($P2W-$P1W)${NC}"
    else
      echo -e "${RED}${BOLD}You lost the battle. ($P2W-$P1W)${NC}"
    fi
    echo -e "${BOLD}================================${NC}"
    exit 0
  fi
